...
const StatisticsCard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [vehicleRes, purchaseRes] = await Promise.all([
          api.get(`/vehicle`),
          api.get(`/purchase`),
        ]);
        ...
        // ============================
        //  VEHICLE API MAINTENANCE
        // ============================
        vehicles.forEach((vehicle) => {
          // Document Expiry
          ["fitness_date", "road_permit_date", "reg_date"].forEach((type) => {
            const rawDate = vehicle[type];
            if (rawDate) {
              const date = dayjs(rawDate);
              const remaining = date.diff(today, "day");
          ...
          // Service (KM)
          if (vehicle.last_service_km && vehicle.service_interval_km && vehicle.current_km) {
            const kmSinceService = vehicle.current_km - vehicle.last_service_km;
            if (kmSinceService >= vehicle.service_interval_km - 500) {
              maintenance.push({
                vehicle: `${vehicle.reg_zone}-${vehicle.reg_no}`,
        ...
        // ============================
        //  PURCHASE API MAINTENANCE
        // ============================
        purchases.forEach((p) => {
          const vehicle = p.vehicle_no || "Unknown Vehicle";

          // ---------- DATE BASED SERVICE ----------
          if (p.next_service_date) {
            const date = dayjs(p.next_service_date);
            const remaining = date.diff(today, "day");

            if (date.isValid() && remaining <= 30 && remaining >= -10) {
              maintenance.push({
                vehicle,
                type: `${p.category?.replace(/_/g, " ")} Service`,
                expireDate: date.format("DD-MM-YYYY"),
                remaining,
              });
            }
          ...
          // ---------- KM BASED SERVICE ----------
          if (p.last_km && p.next_km) {
            const remaining = p.next_km - p.last_km;

            if (remaining <= 500) {
              maintenance.push({
                vehicle,
                type: `${p.category?.replace(/_/g, " ")} (KM Based)`,
                expireDate: `${p.next_km} km`,
                remaining,
              });
            }
          }
      ...
      {data.length > 0 ? (
        <div className="overflow-x-auto max-h-56 overflow-y-auto hide-scrollbar">
          <table className="min-w-full text-sm text-left border border-gray-200">
            ...
            <tbody>
              {data.map((item, i) => (
                <tr key={i} className="text-gray-700 font-semibold border-b border-gray-200">
                  <td className="p-2">{i + 1}</td>
                  <td className="p-2">{item.vehicle}</td>
                  <td className="p-2">{item.type}</td>
                  <td className="p-2">
